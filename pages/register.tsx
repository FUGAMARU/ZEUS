...
//Settings
import { auth, registerUserInformation, checkUserDataExists, getClassReferenceFromString } from "../firebase"
  ...
  const register = async () => {
    if (secondName.current?.value && firstName.current?.value) {
      ...
      const input = {
        ...
        class: await getClassReferenceFromString(selectedClass)
      }
      ...
      if (userIconFlag) {
        ...
        await registerUserInformation(UID, input, compressedFile)
        Router.push("/")
      } else {
        await registerUserInformation(UID, input, null)
        Router.push("/")
      }
    }
  }
